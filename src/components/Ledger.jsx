...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-50 ${bgColor} text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-2`}>
...
export default function Ledger() {
  ...
  const [archivedInvoicesData, setArchivedInvoicesData] = useState([]);
  ...
  useEffect(() => {
    ...
    const unsubArchived = onSnapshot(collection(db, 'archived_invoices'), s =>
      setArchivedInvoicesData(s.docs.map(d => ({ id: d.id, ...d.data() })))
    );
    const unsubPayments = onSnapshot(collection(db, 'payments'), s =>
      setPayments(s.docs.map(d => ({ id: d.id, ...d.data() })))
    );
    return () => { unsubClients(); unsubInvoices(); unsubArchived(); unsubPayments(); };
  }, []);
  ...
  const allArchivedInvoices = archivedInvoicesData.flatMap(doc => doc.invoices || []);
  const allInvoices = [...invoices, ...allArchivedInvoices];
  const clientInvoices = allInvoices.filter(inv => inv.clientId === selectedClientId);
  ...
  const debitReceived = clientInvoices.filter(inv => inv.billType === 'debit').reduce((sum, inv) => {
    const invoiceTotal = inv.total || inv.amount || 0;
    const p = getPaymentsForInvoice(inv.id);
    if (p > 0) return sum + Math.min(p, invoiceTotal);
    if (inv.status === 'paid') return sum + invoiceTotal;
    if (inv.status === 'partial') return sum + (inv.amountReceived || 0);
    return sum;
  }, 0);
  ...
  const isDebit = activeFilter === 'debit' || (activeFilter !== 'credit' && debitQuoted > 0);
  const displayQuoted = isDebit ? debitQuoted : totalQuoted;
  const displayReceived = isDebit ? debitReceived : totalReceived;
  const displayOutstanding = isDebit ? Math.max(0, debitQuoted - debitReceived) : totalOutstanding;
  const displayCollectionRate = isDebit 
    ? (debitQuoted > 0 ? ((debitReceived / debitQuoted) * 100).toFixed(1) : '0.0')
    : collectionRate;
  ...
  const downloadPDF = () => {
    if (!selectedClient) { showToast('Please select a client first', 'error'); return; }

    try {
      const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
      const pw = doc.internal.pageSize.getWidth();
      const ph = doc.internal.pageSize.getHeight();
      const lm = 15, rm = 15, cw = pw - lm - rm;
      let y = 15;

      // Logo area
      doc.setFillColor(79, 70, 229);
      doc.roundedRect(lm, y, cw, 22, 4, 4, 'F');
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(16);
      doc.setFont(undefined, 'bold');
      doc.text('Building India Digital', lm + cw / 2, y + 9, { align: 'center' });
      doc.setFontSize(8);
      doc.setFont(undefined, 'normal');
      doc.text('Client Ledger Statement', lm + cw / 2, y + 16, { align: 'center' });
      y += 30;
      ...
      doc.setFontSize(11);
      doc.setFont(undefined, 'bold');
      doc.text(`Client: ${selectedClient.name}${selectedClient.company ? '  |  ' + selectedClient.company : ''}`, lm, y);
      doc.setFontSize(8);
      doc.setFont(undefined, 'normal');
      doc.setTextColor(130, 130, 130);
      doc.text(`Generated: ${new Date().toLocaleString()}`, pw - rm, y, { align: 'right' });
      y += 10;

      // 3 summary boxes
      const isDebit = activeFilter === 'debit';
      const pdfQuoted = isDebit ? debitQuoted : totalQuoted;
      const pdfReceived = isDebit ? debitReceived : totalReceived;
      const pdfOutstanding = isDebit ? Math.max(0, debitQuoted - debitReceived) : totalOutstanding;
      ...
      boxes.forEach((box, i) => {
        ...
        doc.setFontSize(7);
        doc.setFont(undefined, 'normal');
        doc.text(box.label.toUpperCase(), bx + bw / 2, y + 7, { align: 'center' });
        doc.setFontSize(11);
        doc.setFont(undefined, 'bold');
        doc.text(box.value, bx + bw / 2, y + 17, { align: 'center' });
      });
      y += 32;

      // Invoice table (no status column)
      const tableData = filteredInvoices.map(inv => {
        const invoiceTotal = inv.total || inv.amount || 0;
        const pmt = getPaymentsForInvoice(inv.id);
        const received = pmt > 0 ? Math.min(pmt, invoiceTotal) : inv.status === 'paid' ? invoiceTotal : (inv.amountReceived || 0);
        ...
        return [
          ...
          `Rs. ${formatAmount(invoiceTotal)}`,
          `Rs. ${formatAmount(received)}`,
      ...
      autoTable(doc, {
        ...
        body: tableData,
        theme: 'striped',
        headStyles: { fillColor: [79, 70, 229], textColor: [255, 255, 255], fontSize: 7, fontStyle: 'bold', halign: 'center', cellPadding: 3 },
        ...
        didParseCell: (data) => {
          if (data.section === 'body' && data.column.index === 3) {
            const inv = filteredInvoices[data.row.index];
            data.cell.styles.textColor = inv?.billType === 'debit' ? [200, 0, 0] : inv?.billType === 'credit' ? [0, 140, 60] : [100, 100, 100];
        ...
        didDrawPage: () => {
          ...
          doc.text('Building India Digital — Confidential', lm, ph - 8);
          doc.text(`Page ${doc.internal.getNumberOfPages()}`, pw - rm, ph - 8, { align: 'right' });
        },
      });

      doc.save(`Ledger_${selectedClient.name.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`);
      showToast(`PDF downloaded for ${selectedClient.name}`, 'success');
    } catch (err) {
      ...
      <div className="px-3 py-4 sm:px-5 sm:py-6 md:px-8 md:py-8 max-w-[1400px] mx-auto space-y-4 sm:space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-5">
          ...
          <div className="relative">
            <button onClick={() => setShowDropdown(!showDropdown)} className="w-full flex items-center justify-between gap-3 px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 hover:border-indigo-400 hover:bg-white transition-all text-left">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${selectedClient ? 'bg-gradient-to-br from-indigo-400 to-purple-500' : 'bg-slate-200'}`}>
                  {selectedClient ? <span className="text-sm font-black text-white">{selectedClient.name?.charAt(0)?.toUpperCase()}</span> : <User className="w-4 h-4 text-slate-400" />}
                </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {selectedClient && (
                  <button onClick={e => { e.stopPropagation(); setSelectedClientId(''); setActiveFilter('all'); }} className="p-1 rounded-lg hover:bg-slate-200 text-slate-400 transition-colors">
              ...
              <div className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                <div className="p-3 border-b border-slate-100">
                  <div className="relative">
                    ...
                    <input autoFocus type="text" placeholder="Search by name or company…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none" onClick={e => e.stopPropagation()} />
                  </div>
                </div>
                <div className="max-h-60 overflow-y-auto">
                  {filteredClients.length === 0 ? (
                    ...
                    <button key={client.id} className={`w-full px-4 py-3 flex items-center gap-3 text-left hover:bg-indigo-50 transition-colors ${selectedClientId === client.id ? 'bg-indigo-50' : ''}`} onClick={() => { setSelectedClientId(client.id); setShowDropdown(false); setSearchTerm(''); setActiveFilter('all'); }}>
        ...
        {selectedClient && (
          <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-4 mt-1.5">
                      {selectedClient.email && <span className="text-xs text-indigo-200 flex items-center gap-1.5"><Mail className="w-3.5 h-3.5 flex-shrink-0" />{selectedClient.email}</span>}
                      {selectedClient.phone && <span className="text-xs text-indigo-200 flex items-center gap-1.5"><Phone className="w-3.5 h-3.5 flex-shrink-0" />{selectedClient.phone}</span>}
                    </div>
                  </div>
                </div>
            ...
            {/* KPI Cards */}
            <div className={`grid grid-cols-2 ${isDebit ? 'lg:grid-cols-3' : 'lg:grid-cols-4'} gap-3`}>
              {[
                { 
                  label: isDebit ? 'Total Billed' : 'Total Quoted', 
                  value: `₹${formatAmount(displayQuoted)}`, 
                  color: 'blue', 
                ...
                { 
                  label: isDebit ? 'Total Paid' : 'Total Received', 
                  value: `₹${formatAmount(displayReceived)}`, 
                  color: 'emerald', 
                ...
                { 
                  ...
                  barW: displayQuoted > 0 ? `${Math.min(100, (displayOutstanding / displayQuoted) * 100)}%` : '0%' 
                },
                !isDebit && { 
                  label: 'Collection Rate', 
                  value: `${displayCollectionRate}%`, 
                  color: 'violet', 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Quoted Amount</p><p className="text-lg sm:text-xl font-black text-green-600">₹{formatAmount(creditQuoted)}</p></div>
                  <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Received Amount</p><p className="text-lg sm:text-xl font-bold text-emerald-600">₹{formatAmount(creditReceived)}</p></div>
                  <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Outstanding</p><p className="text-lg sm:text-xl font-bold text-rose-600">₹{formatAmount(Math.max(0, creditQuoted - creditReceived))}</p></div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'Paid', count: paidCount, color: 'emerald', icon: <CheckCircle className="w-4 h-4 text-emerald-600" /> },
                { label: 'Partial', count: partialCount, color: 'amber', icon: <Clock className="w-4 h-4 text-amber-600" /> },
                { label: 'Unpaid', count: unpaidCount, color: 'rose', icon: <AlertCircle className="w-4 h-4 text-rose-600" /> },
              ].map(s => (
                <div key={s.label} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                  <div className="flex items-center gap-2.5 mb-3">
                    <div className={`p-2 bg-${s.color}-50 rounded-xl`}>{s.icon}</div>
            ...
            {!isDebit && (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                ...
                <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden mb-2.5">
                  <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 h-4 rounded-full transition-all duration-700" style={{ width: `${Math.min(100, parseFloat(collectionRate))}%` }} />
                </div>
            ...
            {/* Invoice Table with Download Button inside */}
                ...
                <div className="flex gap-1.5 flex-wrap mt-4">
                  {filterButtons.map(f => (
                    <button key={f.key} onClick={() => setActiveFilter(f.key)}
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all flex items-center gap-1.5 ${activeFilter === f.key ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-md shadow-indigo-200' : 'bg-slate-50 text-slate-600 border border-slate-200 hover:border-indigo-300 hover:text-indigo-600'}`}>
                      {f.label}
                      <span className={`w-4 h-4 flex items-center justify-center rounded-full text-[9px] font-black ${activeFilter === f.key ? 'bg-white/25 text-white' : 'bg-slate-200 text-slate-500'}`}>{f.count}</span>
                    </button>
                  ))}
                </div>
              </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {filteredInvoices.map((invoice, idx) => {
                          ...
                          const pmt = getPaymentsForInvoice(invoice.id);
                          const received = pmt > 0 ? Math.min(pmt, invoiceTotal) : invoice.status === 'paid' ? invoiceTotal : (invoice.amountReceived || 0);
                          ...
                          return (
                            <tr key={invoice.id} className={`border-b border-slate-50 hover:bg-indigo-50/30 transition-colors ${idx % 2 === 0 ? '' : 'bg-slate-50/30'}`}>
                              ...
                              <td className="px-4 py-3.5"><div className="flex items-center gap-1.5 text-xs text-slate-500 whitespace-nowrap"><Calendar className="w-3 h-3 text-slate-400" />{invoice.date || '—'}</div></td>
                              ...
                              <td className="px-4 py-3.5"><Badge styles={billS} label={(invoice.billType || 'none').charAt(0).toUpperCase() + (invoice.billType || 'none').slice(1)} /></td>
                              <td className="px-4 py-3.5"><span className="text-sm font-black text-slate-800 whitespace-nowrap">₹{formatAmount(invoiceTotal)}</span></td>
                              <td className="px-4 py-3.5"><span className="text-xs font-bold text-emerald-600 whitespace-nowrap">₹{formatAmount(received)}</span></td>
                              <td className="px-4 py-3.5"><span className={`text-xs font-bold whitespace-nowrap ${outstanding > 0 ? 'text-rose-600' : 'text-slate-400'}`}>{outstanding > 0 ? `₹${formatAmount(outstanding)}` : '—'}</span></td>
                              <td className="px-4 py-3.5"><Badge styles={statusS} label={invoice.status || 'unpaid'} /></td>
                            </tr>
                          );
                        })}
                      </tbody>
                        ...
                        <div key={invoice.id} className="p-4 hover:bg-indigo-50/20 transition-colors">
                          ...
                          {services.length > 0 && (
                            <div className="mb-3 bg-slate-50 rounded-xl px-3 py-2 border border-slate-100">
                              {services.slice(0, 2).map((s, i) => <p key={i} className="text-xs text-slate-600 truncate">{i + 1}. {s.name} — ₹{formatAmount(s.amount || 0)}</p>)}
                              {services.length > 2 && <p className="text-[10px] text-indigo-500 font-semibold mt-0.5">+{services.length - 2} more</p>}
                            </div>
                          )}
                          <div className="grid grid-cols-3 gap-2">
                            <div className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 text-center">
                              <p className="text-[9px] text-slate-400 font-semibold mb-0.5">
                                {invoice.billType === 'debit' ? 'TOTAL BILLED' : 'QUOTED'}
                              </p>
                            ...
                            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-2.5 text-center">
                              <p className="text-[9px] text-emerald-500 font-semibold mb-0.5">
                                {invoice.billType === 'debit' ? 'TOTAL PAID' : 'RECEIVED'}
                              </p>
                            ...
                            <div className={`rounded-xl p-2.5 text-center border ${outstanding > 0 ? 'bg-rose-50 border-rose-100' : 'bg-slate-50 border-slate-100'}`}>
                              <p className={`text-[9px] font-semibold mb-0.5 ${outstanding > 0 ? 'text-rose-500' : 'text-slate-400'}`}>
                                {invoice.billType === 'debit' ? 'REMAINING BALANCE' : 'OUTSTANDING'}
                              </p>
              ...
              {filteredInvoices.length > 0 && (
                <div className="px-4 sm:px-5 py-3.5 border-t border-slate-100 bg-gradient-to-r from-indigo-50 to-purple-50 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex gap-4 text-xs">
                    <span className="text-slate-500">{isDebit ? 'Total Billed' : 'Total Quoted'}: <span className="font-black text-slate-800">₹{formatAmount(filteredInvoices.reduce((s, i) => s + (i.total || i.amount || 0), 0))}</span></span>
                    <span className="text-slate-500">{isDebit ? 'Total Paid' : 'Total Received'}: <span className="font-black text-emerald-600">₹{formatAmount(filteredInvoices.reduce((s, i) => {
