...
export default function Ledger() {
  ...
  const downloadPDF = () => {
    ...
    try {
      ...
      // 3 summary boxes
      const boxes = [
        { label: 'Total Quoted', value: `Rs. ${formatAmount(totalQuoted)}`, fill: [79, 70, 229], text: [255, 255, 255] },
        { label: 'Total Received', value: `Rs. ${formatAmount(totalReceived)}`, fill: [16, 185, 129], text: [255, 255, 255] },
        { label: 'To Be Collected', value: `Rs. ${formatAmount(totalOutstanding)}`, fill: [244, 63, 94], text: [255, 255, 255] },
      ];
      const bw = (cw - 8) / 3;
      boxes.forEach((box, i) => {
        const bx = lm + i * (bw + 4);
      ...
      autoTable(doc, {
        startY: y,
        head: [['Invoice #', 'Date', 'Services', 'Type', 'Quoted', 'Received', 'Remaining']],
        body: tableData,
        theme: 'striped',
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                ...
                <div className="space-y-2">
                  <div><p className="text-xs text-slate-500 mb-1">Quoted Amount</p><p className="text-2xl font-black text-red-600">₹{formatAmount(debitQuoted)}</p></div>
                  <div><p className="text-xs text-slate-500 mb-1">Received Amount</p><p className="text-lg font-bold text-emerald-600">₹{formatAmount(debitReceived)}</p></div>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                ...
                <div className="space-y-2">
                  <div><p className="text-xs text-slate-500 mb-1">Quoted Amount</p><p className="text-2xl font-black text-green-600">₹{formatAmount(creditQuoted)}</p></div>
                  <div><p className="text-xs text-slate-500 mb-1">Received Amount</p><p className="text-lg font-bold text-emerald-600">₹{formatAmount(creditReceived)}</p></div>
                </div>
                <div className="mt-3 pt-3 border-t border-slate-100">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-slate-500">Collection Progress</span>
