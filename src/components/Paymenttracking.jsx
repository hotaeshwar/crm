...
export default function PaymentTracking() {
  ...
  // Track previous payments snapshot to detect any change (not just length)
  ...
  useEffect(() => {
    ...
    const unsubPayments = onSnapshot(
      ...
      (snapshot) => setPayments(snapshot.docs.map(d => ({ id: d.id, ...d.data() }))),
      (err) => setError(err.message)
    );
  ...
  // ─────────────────────────────────────────────────────────────────────
  // DERIVED VALUES FOR SELECTED INVOICE IN FORM
  // Uses payments collection first; falls back to invoice stored fields
  // so that manually-set partial/paid statuses show correctly.
  // ─────────────────────────────────────────────────────────────────────
  ...
  const totalPaidForSelected = selectedInvoice
    ? (() => {
        const fromPayments = getTotalPaidForInvoice(selectedInvoice.id);
        return fromPayments > 0 ? fromPayments : (selectedInvoice.amountReceived || 0);
      })()
    : 0;
  ...
  const remainingForSelected = selectedInvoice
    ? (() => {
        const fromPayments = getTotalPaidForInvoice(selectedInvoice.id);
        if (fromPayments > 0) return getRemainingAmount(selectedInvoice);
        if (selectedInvoice.remainingAmount != null) return Math.max(0, selectedInvoice.remainingAmount);
        return Math.max(0, (selectedInvoice.total || selectedInvoice.amount || 0));
      })()
    : 0;

  // Status: from payments collection if records exist, else trust invoice.status
  const paymentStatus = selectedInvoice
    ? (() => {
        const fromPayments = getTotalPaidForInvoice(selectedInvoice.id);
        return fromPayments > 0
  ...
  // ─────────────────────────────────────────────────────────────────────
  // UPDATE INVOICE STATUS — only touches invoices with payment records
  // Never overwrites manually-set statuses for invoices with no payments
  // ─────────────────────────────────────────────────────────────────────
  const updateInvoiceStatus = async (invoiceId, paymentsSnapshot = payments) => {
    ...
    if (invoicePayments.length === 0) return; // no records → don't touch

    const totalPaid    = invoicePayments.reduce((sum, p) => sum + (p.amount || 0), 0);
    const invoiceTotal = invoice.total || invoice.amount || 0;

    let newStatus = 'unpaid';
    if (totalPaid > 0 && totalPaid < invoiceTotal) newStatus = 'partial';
    else if (totalPaid >= invoiceTotal) newStatus = 'paid';

    const unchanged =
      invoice.status === newStatus &&
      invoice.amountReceived === totalPaid &&
      invoice.remainingAmount === Math.max(0, invoiceTotal - totalPaid);
    if (unchanged) return;

    await updateDoc(doc(db, 'invoices', invoiceId), {
      status: newStatus,
      amountReceived: totalPaid,
      remainingAmount: Math.max(0, invoiceTotal - totalPaid)
    });
  };

  // ─────────────────────────────────────────────────────────────────────
  // SYNC — only invoices that have payment records
  // ─────────────────────────────────────────────────────────────────────
  useEffect(() => {
    ...
    (async () => {
      for (const id of invoiceIdsWithPayments) await updateInvoiceStatus(id, payments);
    })();
  }, [payments, invoices]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!inv) { setError('Please select a valid invoice'); return; }

      const paymentAmount = parseFloat(form.amount);
      if (isNaN(paymentAmount) || paymentAmount <= 0) { setError('Please enter a valid payment amount'); return; }

      const invoiceTotal = inv.total || inv.amount || 0;
      const paidSoFar    = getTotalPaidForInvoice(inv.id);

      if (editId) {
        const oldAmount      = payments.find(p => p.id === editId)?.amount || 0;
        ...
        await updateDoc(doc(db, 'payments', editId), {
          invoiceId: form.invoiceId, amount: paymentAmount,
          paymentDate: form.paymentDate, method: form.method, updatedAt: new Date()
        });
        ...
        await addDoc(collection(db, 'payments'), {
          invoiceId: form.invoiceId, amount: paymentAmount,
          paymentDate: form.paymentDate, method: form.method, createdAt: new Date()
        });
        await updateInvoiceStatus(inv.id);
      }

      setForm({ invoiceId: '', paymentDate: new Date().toISOString().split('T')[0], method: 'bank', amount: '' });
  ...
  // Use invoice.status directly — it reflects both manual + payment-synced values
        ...
        <form onSubmit={handleSubmit} className="bg-white p-3 sm:p-4 md:p-5 lg:p-6 rounded-xl sm:rounded-2xl shadow-lg mb-4 sm:mb-6 border border-slate-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
                  ...
                  <div className="absolute z-50 w-full mt-1 bg-white border border-slate-300 rounded-lg shadow-lg max-h-48 sm:max-h-60 overflow-y-auto">
                    ...
                    {filteredInvoicesForDropdown.length > 0 ? (
                      filteredInvoicesForDropdown.map((invoice) => {
                        ...
                        // Trust invoice.status (includes manually-set values)
                        const invStatus   = invoice.status || 'unpaid';
                        const paidDisplay = fromPmts > 0 ? fromPmts : (invoice.amountReceived || 0);
                        const remDisplay  = fromPmts > 0
                          ? Math.max(0, invTotal - fromPmts)
                          : Math.max(0, invoice.remainingAmount ?? invTotal);

                        // ── KEY FIX: auto-fill based on status ──
                        // partial → remaining (NOT full total)
                        // unpaid  → full total
                        // paid    → empty
                        const handleSelect = () => {
                          let autoAmount = '';
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {invStatus === 'partial' && (
                                  <div className="text-[9px] sm:text-[10px] text-amber-600 mt-0.5">
                                    Paid: ₹{formatAmount(paidDisplay)} · Remaining: ₹{formatAmount(remDisplay)}
                                  </div>
                                )}
                              </div>
            ...
            <div>
              ...
              <div className="relative">
                <DollarSign className="absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-slate-400" />
            ...
            <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-gradient-to-br from-slate-50 to-indigo-50 rounded-lg sm:rounded-xl border border-slate-200">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                ...
                <div className="bg-white rounded-lg p-2 sm:p-2.5 border border-slate-200">
                  ...
                  {/* Shows REMAINING (what still needs to be paid), not the full invoice total */}
