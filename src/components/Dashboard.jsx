...
export default function Dashboard() {
  ...
  const [payments, setPayments] = useState([]);   // ← reads payments collection
  ...
  // ── Firestore listeners — now includes payments collection ──
  useEffect(() => {
    ...
    // NEW: listen to payments so dashboard numbers stay live
  ...
  // ── Stats calculation — uses payments collection as source of truth ──
  // Priority:
  //   1. If invoice has payment records → sum those payments
  //   2. If no payment records but status='paid' → full invoice total
  //   3. If no payment records but status='partial' → invoice.amountReceived (manually set)
  //   4. Otherwise → 0 collected
  useEffect(() => {
    // Helper: total paid from payments collection for one invoice
    ...
    invoices.forEach(inv => {
      ...
      // How much collected for this invoice
      const fromPayments = getPaidFromPayments(inv.id);
      let collected = 0;
      if (fromPayments > 0) {
        // Payments collection is source of truth
        collected = Math.min(fromPayments, invoiceTotal);
      } else if (inv.status === 'paid') {
        // No payment records, but marked paid manually
        collected = invoiceTotal;
      } else if (inv.status === 'partial') {
        // No payment records, manually set partial
  ...
  }, [clients, invoices, payments]); // payments in deps → recalculates whenever payment recorded
            ...
            {invoices.length === 0 ? (
              ...
              <>
                {recentInvoices.map(invoice => {
                  const client = clients.find(c => c.id === invoice.clientId);
                  const amount = invoice.total || invoice.subtotal || invoice.amount || 0;
                  // For partial invoices: show collected vs total in the list
