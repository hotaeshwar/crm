...
export default function InvoiceManagement() {
  ...
  const handleSaveEdit = async (invoiceId) => {
    try {
      ...
      // FIX: auto-set amountReceived to total when status is paid
      const amountReceived = editForm.status === 'paid'
        ? total
        : parseFloat(editForm.amountReceived) || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    const total = subtotal + taxAmount;
    // FIX: auto-set amountReceived to total when status is paid
    const amountReceived = form.status === 'paid'
      ? total
      : parseFloat(form.amountReceived) || 0;
  ...
  const downloadPDF = async (invoice) => {
    ...
    // Services Table with Border Boxes
    ...
    // Amount Received (for paid and partial)
    ...
    // Remaining Amount (for unpaid and partial)
  ...
  const deleteArchivedInvoice = async (archiveId, invoiceIndex) => {
    ...
    try {
      ...
      alert('Archived invoice deleted successfully!');
    } catch (error) {
      console.error('Error deleting archived invoice:', error);
      alert('Error deleting archived invoice. Please try again.');
    }
  };
                ...
                <div className="p-4">
                  <div className="grid grid-cols-7 gap-2">
                    ...
                    {renderMonthlyCalendar().map((day, index) => {
                      ...
                      const totalAmount = day.invoices.reduce((sum, inv) => sum + (inv.total || 0), 0);
                        ...
                        <button
                          ...
                          className={`min-h-[80px] p-2 rounded-lg border-2 transition-all duration-200 ${
                            isSelected
                              ...
                              : hasInvoices
                              ? 'border-blue-200 bg-blue-25 hover:border-blue-400'
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <span className={`text-sm font-bold ${
                              isToday ? 'text-purple-600' : isSelected ? 'text-blue-600' : 'text-slate-700'
                            }`}>
                              {day.date.getDate()}
                            </span>
                            {hasInvoices && (
                        ...
                        {yearData.monthlySummary.map((month, monthIndex) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {hasInvoices ? (
                                <>
                                  <div className="text-lg font-bold text-blue-600">{month.count}</div>
                                  <div className="text-xs text-slate-600">₹{formatAmount(month.total)}</div>
                                </>
                              ) : (
              ...
              <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                ...
                {filteredInvoices.length > 0 ? (
                  ...
                  <div className="p-8 text-center">
                    ...
                    <p className="text-slate-500">No invoices found for this date</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-4 sm:space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 sm:gap-3 mb-1.5">
                    <div className="p-2 sm:p-2.5 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-lg sm:rounded-xl">
                  ...
                  <p className="text-xs sm:text-sm text-slate-600 ml-11 sm:ml-14">
                    View and manage archived monthly data from Firebase
                  </p>
                </div>
                
                <div className="flex gap-2">
                  <button
                    onClick={loadArchivedInvoices}
                  ...
                  <button
                    onClick={archiveMonthlyData}
                    className="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all flex items-center gap-2 text-sm font-medium shadow-md"
                  >
                    <FolderArchive className="w-4 h-4" />
                    Archive Now
                  </button>
            ...
            {loadingArchive ? (
              <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-12 text-center">
                ...
                <p className="text-sm text-slate-500 mt-4">Loading archived data...</p>
              </div>
            ) : archivedInvoices.length === 0 ? (
              <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-12 text-center">
                <Archive className="w-16 h-16 text-slate-300 mx-auto mb-4" />
                <h3 className="text-lg font-bold text-slate-700 mb-2">No Archived Data</h3>
                <p className="text-sm text-slate-500 max-w-md mx-auto">
                  No archived invoices found in Firebase. Click the "Archive Now" button to archive previous month's data.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {archivedInvoices.map((archive) => (
                  <div key={archive.id} className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-purple-500 to-indigo-600 p-4 sm:p-5">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        <div>
                          ...
                          <p className="text-purple-100 text-xs sm:text-sm">
                            Archived: {archive.archivedDate ? new Date(archive.archivedDate.seconds * 1000).toLocaleDateString() : 'N/A'}
                          </p>
                        </div>
                        
                        <div className="flex gap-2">
                          <button
                            onClick={() => exportToExcel(archive.invoices || [], `archived_${archive.monthName}_${archive.year}`)}
                            className="px-3 py-1.5 bg-white/20 backdrop-blur text-white rounded-lg hover:bg-white/30 transition-all flex items-center gap-1.5 text-xs sm:text-sm font-medium"
                          >
                          ...
                          <button
                            onClick={() => viewArchiveMonth(archive)}
                            className={`px-3 py-1.5 bg-white text-purple-600 rounded-lg hover:bg-purple-50 transition-all flex items-center gap-1.5 text-xs sm:text-sm font-medium ${
                          ...
                          >
                            ...
                            {selectedArchiveMonth === archive.monthName && selectedArchiveYear === archive.year ? 'Hide' : 'View'}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-3 sm:gap-4 mt-4">
                        <div className="bg-white/10 backdrop-blur rounded-lg p-2 sm:p-3">
                          <div className="text-white/80 text-[10px] sm:text-xs">Invoices</div>
                          <div className="text-lg sm:text-xl font-bold text-white">
                            {archive.invoiceCount || (archive.invoices?.length || 0)}
                          </div>
                        </div>
                        <div className="bg-white/10 backdrop-blur rounded-lg p-2 sm:p-3">
                          <div className="text-white/80 text-[10px] sm:text-xs">Total Amount</div>
                          <div className="text-lg sm:text-xl font-bold text-white">
                            ₹{formatAmount(archive.totalAmount || 0)}
                          </div>
                        </div>
                        <div className="bg-white/10 backdrop-blur rounded-lg p-2 sm:p-3">
                          <div className="text-white/80 text-[10px] sm:text-xs">Firebase ID</div>
                          <div className="text-xs sm:text-sm font-bold text-white truncate" title={archive.id}>
                    ...
                    {/* Archived Invoices List - Shown when expanded */}
                    {selectedArchiveMonth === archive.monthName && selectedArchiveYear === archive.year && (
                      <div className="p-4 sm:p-5 border-t border-slate-200">
                        <h4 className="text-sm sm:text-base font-bold text-slate-800 mb-3 flex items-center gap-2">
                          ...
                          Invoices from {archive.monthName} {archive.year}
                        </h4>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 mb-4">
                          <div className="flex-1 relative">
                            ...
                            <input
                              ...
                              className="w-full pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                            />
                          </div>
                          <select
                              ...
                              {filterArchivedInvoices().map((inv, idx) => {
                                ...
                                return (
                                  <tr key={idx} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-3 text-xs font-bold text-blue-600">₹{formatAmount(inv.total || 0)}</td>
                                    <td className="px-4 py-3">
                                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${getStatusColor(inv.status)}`}>
                                    ...
                                    <td className="px-4 py-3">
                                      <div className="flex gap-1">
                                        ...
                                        <button
                                          onClick={() => restoreFromArchive(inv)}
                                          className="p-1.5 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200"
                                          title="Restore to main invoices"
                                        >
                                          <History className="w-4 h-4" />
                                        </button>
                                        <button
                                          onClick={() => deleteArchivedInvoice(archive.id, archive.invoices.findIndex(i => i.invoiceNumber === inv.invoiceNumber))}
                                          className="p-1.5 bg-rose-100 text-rose-700 rounded-lg hover:bg-rose-200"
                              ...
                              <div key={idx} className="bg-slate-50 rounded-lg p-3 border border-slate-200">
                                ...
                                <div className="grid grid-cols-2 gap-2 mb-2">
                                  <div>
                                    ...
                                    <div className="text-sm font-semibold text-slate-700">{client?.name || 'N/A'}</div>
                                  </div>
                                  <div>
                                ...
                                <div className="flex justify-between items-center pt-2 border-t border-slate-200">
                                  <div>
                                    ...
                                    <div className="text-base font-bold text-blue-600">₹{formatAmount(inv.total || 0)}</div>
                                  </div>
                                  <div className="flex gap-1">
                                    <button
                                      onClick={() => downloadPDF(inv)}
                        ...
                        {filterArchivedInvoices().length > 0 && (
                          <div className="mt-4 pt-4 border-t border-slate-200 flex justify-between items-center">
                            <span className="text-sm text-slate-600">
                              Showing {filterArchivedInvoices().length} of {archive.invoices?.length || 0} invoices
                            </span>
                            <button
                              onClick={() => exportToExcel(filterArchivedInvoices(), `archived_${archive.monthName}_${archive.year}_filtered`)}
                              className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-lg hover:from-emerald-700 hover:to-green-700 transition-all flex items-center gap-2 text-sm font-medium shadow-md"
                            >
                              <Download className="w-4 h-4" />
                              Export Filtered
                            </button>
