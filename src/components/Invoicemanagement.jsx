...
export default function InvoiceManagement() {
  ...
  const [archiveSelectValue, setArchiveSelectValue] = useState('');

  // Unique month/year options from current active invoices
  const activeInvoiceMonths = useMemo(() => {
    const options = [];
    invoices.forEach(inv => {
      ...
      if (!options.some(o => o.month === m && o.year === y)) {
        options.push({ month: m, year: y, label: `${new Date(2000, m).toLocaleString('default', { month: 'long' })} ${y}` });
      }
    });
    // Sort options chronologically (most recent first)
    options.sort((a, b) => {
  ...
  const archiveMonthlyData = async (monthToArchive, yearToArchive) => {
    try {
      let targetMonth = monthToArchive;
      let targetYear = yearToArchive;
      ...
      const monthlyInvoices = getInvoicesByMonthYear(targetMonth, targetYear);
      const monthName = new Date(2000, targetMonth).toLocaleString('default', { month: 'long' });
      
      if (monthlyInvoices.length > 0) {
        const existingArchive = archivedInvoices.find(
          a => a.month === targetMonth && a.year === targetYear
        );
        
        if (existingArchive) {
          if (!confirm(`Data for ${monthName} ${targetYear} is already archived. Do you want to archive again?`)) {
            return;
          }
        ...
        const fileName = `invoices_${monthName}_${targetYear}`;
        exportToExcel(monthlyInvoices, fileName);
        
        // Also delete any payments associated with these archived invoices
        const paymentsSnapshot = await getDocs(collection(db, 'payments'));
        const invoiceIdsToArchive = new Set(monthlyInvoices.map(inv => inv.id));
        const deletePaymentPromises = paymentsSnapshot.docs
          .filter(doc => invoiceIdsToArchive.has(doc.data().invoiceId))
        ...
        setArchiveSelectValue('');
        
        showToast(`Monthly data for ${monthName} ${targetYear} archived successfully!`, 'success');
      } else {
        showToast(`No invoices found for ${monthName} ${targetYear}.`, 'info');
      }
    } catch (error) {
      console.error(error);
  ...
  const renderYearlyView = () => {
    ...
    const invoiceYears = [...new Set(invoices.map(inv => 
      parseLocalDate(inv.date).getFullYear()
    ))];
    ...
    return invoiceYears.map(year => {
      ...
      const monthlySummary = Array(12).fill().map((_, month) => {
        const monthInvoices = yearInvoices.filter(inv => 
          parseLocalDate(inv.date).getMonth() === month
        );
  ...
  const downloadPDF = async (invoice) => {
    ...
    pdfDoc.setFont(undefined, 'bold');
    pdfDoc.text('INVOICE', leftMargin + contentWidth/2, yPos + 8, { align: 'center' });
    yPos += 22;
    ...
    if (isNA(invoice.paymentDays)) {
      ...
      const dueDate = parseLocalDate(invoice.date);
      ...
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
        ...
        {activeTab === 'invoices' && (
          <>
            ...
            <div className="flex flex-col sm:flex-row items-end sm:items-center justify-end gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex flex-col w-full sm:w-auto">
                <label className="text-xs font-semibold text-slate-600 mb-1">Select Month to Archive</label>
                <select
                  value={archiveSelectValue}
                  onChange={(e) => setArchiveSelectValue(e.target.value)}
                  className="border border-slate-300 rounded-lg px-3 py-2 text-sm bg-slate-50 focus:ring-2 focus:ring-emerald-500 focus:border-transparent w-full sm:w-48 text-slate-800"
                >
                  <option value="">Choose Month...</option>
                  {activeInvoiceMonths.map(opt => (
              ...
              <button
                onClick={() => {
                  if (!archiveSelectValue) {
                    showToast('Please select a month to archive', 'error');
                    return;
                  }
                  const [m, y] = archiveSelectValue.split('-').map(Number);
                  archiveMonthlyData(m, y);
                }}
                className="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-lg hover:from-emerald-700 hover:to-green-700 shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 text-sm font-semibold h-fit self-end"
              >
                <FolderArchive className="w-4 h-4" />
                Archive Selected Month
              </button>
            </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 sm:gap-3 mb-1.5">
                    <div className="p-2 sm:p-2.5 bg-gradient-to-br from-purple-50 to-indigo-600 rounded-lg sm:rounded-xl"><Archive className="w-5 h-5 sm:w-6 sm:h-6 text-white" /></div>
                ...
                <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
                  <div className="flex flex-col w-full sm:w-auto">
                    ...
                    <select
                      value={archiveSelectValue}
                      onChange={(e) => setArchiveSelectValue(e.target.value)}
                      className="border border-slate-300 rounded-lg px-3 py-2 text-sm bg-slate-50 focus:ring-2 focus:ring-purple-500 focus:border-transparent w-full sm:w-48 text-slate-800"
                    >
                      <option value="">Choose Month...</option>
                      {activeInvoiceMonths.map(opt => (
                        <option key={`${opt.month}-${opt.year}`} value={`${opt.month}-${opt.year}`}>
                  ...
                  <div className="flex gap-2 w-full sm:w-auto self-end">
                    <button onClick={loadArchivedInvoices} className="flex-1 sm:flex-initial px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-all flex items-center justify-center gap-2 text-sm font-medium shadow-md" disabled={loadingArchive}>
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-initial px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 text-sm font-medium shadow-md border border-emerald-600"
                    >
                      <FolderArchive className="w-4 h-4" />
                      Archive Month
                    </button>
