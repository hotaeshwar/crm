...
import { LayoutDashboard, Users, FileText, Wallet, Menu, X, BookOpen, Megaphone, LogOut, UserCircle } from 'lucide-react';
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-lg">
        <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 xl:px-12 2xl:max-w-[1920px] 2xl:mx-auto">
          <div className="flex items-center justify-between h-14 sm:h-16 md:h-18 lg:h-20 gap-2 xl:gap-4">
            ...
            {/* Desktop — tab pills (centre) */}
            <div className="hidden lg:flex items-center gap-1.5 xl:gap-2 bg-slate-100 p-1.5 rounded-full flex-shrink-0">
              {tabs.map(tab => {
                const Icon = tab.icon;
                return (
                  <button
                    ...
                    className={`px-3 xl:px-5 py-2 rounded-full flex items-center gap-1.5 xl:gap-2 font-semibold text-xs xl:text-sm whitespace-nowrap transition-all ${
                      activeTab === tab.id
            ...
            {/* Desktop — compact user pill with hover dropdown (right) */}
            <div className="hidden lg:block relative flex-shrink-0 group">
              {/* User pill — more compact */}
              <div className="flex items-center gap-1.5 px-2.5 xl:px-3 py-1.5 bg-indigo-50 border border-indigo-200 rounded-full cursor-pointer group-hover:bg-indigo-100 group-hover:border-indigo-300 group-hover:shadow-md transition-all">
                <UserCircle className="w-3.5 h-3.5 xl:w-4 xl:h-4 text-indigo-600 flex-shrink-0" />
                <span className="text-[11px] xl:text-xs font-semibold text-indigo-700 max-w-[120px] xl:max-w-[200px] truncate">
                  {user.email}
                </span>
                {/* Chevron indicator */}
                <svg className="w-2.5 h-2.5 text-indigo-400 flex-shrink-0 transition-transform duration-200 group-hover:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
              </div>

              {/* Dropdown — appears on hover */}
              <div className="absolute right-0 top-full mt-2 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-1 group-hover:translate-y-0 transition-all duration-200 ease-out z-50">
                <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 text-rose-600 hover:bg-rose-50 font-semibold text-sm transition-colors"
                  >
            ...
            {/* Tablet — icons + compact logout area */}
